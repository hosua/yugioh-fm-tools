...
/* Parameters for YFMCardListTable
 * header="your header"
 * subheader="your subheader" (optional)
 * post='/path/to/post'
 */
function YFMCardListTable(props) {
...
function YFMCardDropTable(props) {
    const [data, setData] = useState([]); // data of the table
    const [filteredData, setFilteredData] = useState([]); // the filtered table
    const [dataFilter, setDataFilter] = useState(''); // string to filter by
    const [sortCol, setSortCol] = useState(null);
    ...
    const tableRows = sortedData.map((entry, index) => (
        <tr key={index}>
            {columnHeaders.map((key) => (<td key={key}>{entry[key]}</td>
            ))}
        </tr>
    ));
...
function YFMSelectDuelist() {
    ...
    console.log(JSON.stringify(duelists));

    return (
        <Container data-bs-theme="dark">
            <h2 style={{ textAlign: 'center' }}>Select Duelist</h2>
